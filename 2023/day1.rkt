...
;; String -> Num
(define (calibrate str)
  (let ([lines (string-split str "\n")])
    (apply + ;; sum
           (map (λ(s)
                  ;; convert word numbers to digits, then filter non-digits.
                  (let ([str (filter digit? (string->list (nums->digits s)))])
                    ;; Concatenate first and last
                    (string->number (list->string (list (first str) (last str))))))
                lines))))
...
(define WORDS (list "zero" "one" "two" "three" "four" "five" "six" "seven" "eight" "nine"))
(define SDROW (map string-reverse WORDS))

;; String -> String
;; Converts a word-form number into a string of the relevant digit.
;; If not a word-form number, return the original string.
(define (word->num str)
  (let ([i (or (index-of WORDS str) (index-of SDROW str))])
    (if i (number->string i) str)))

;; WORDS and SDROW as disjunctive regexps.
(define RWORDS (string-join WORDS "|"))
(define RSDROW (string-reverse RWORDS))

;; String -> String
;; Returns the original string, except the first occurrence of a word-form
;; has been replaced with its digit, if any.
...
;; String -> String
;; The same as the above, except it targets the *last* occurrence.
(define (last-word->digit str)
  (let* ([rts (string-reverse str)]
         [word (regexp-match RSDROW rts)])
    (if word
        (let ([word (first word)])
          ;; It's not necessary to append the last letter,
          ;; because we will run this function second.
          (string-reverse (regexp-replace word rts (word->num word))))
        str)))

;; String -> String
;; Driver for the word->digit replacers.
(define (nums->digits str)
  (last-word->digit (first-word->digit str)))

(define test "51591twosix4dhsxvgghxq
425nine
llvmhjtr8nbbhrfone
...
(println (string-append "Calibrate(test): " (number->string (calibrate test))))
(println "Expected:        55413")